...
- **Three retrieval channels** in `RagQueryService.queryVectorDatabase`: rubrics (`deterministic` exact `(readinessType, level)` key lookup by default; `semantic` pgvector nearest-neighbour exists only because SDD §3.2 specifies it, gated by `AI_RAG_RUBRIC_MODE`), business frameworks (always semantic), peers (the pre-existing mechanism, unchanged).
