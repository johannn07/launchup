<script lang="ts">
  ...
  // The ring sweeps to the score in step with the number counting up; both
  // start when the gauge is first on screen. SSR and no-JS draw it finished.
  const ring = new Tween(0, { duration: SETTLE, easing: cubicOut });
  let mounted = $state(false);
  onMount(() => (mounted = true));
  const sweep = $derived(mounted ? ring.current : score);
  const gauge = $derived(
    `background: conic-gradient(${band.colour} 0deg ${sweep * 3.6}deg, #17213a ${sweep * 3.6}deg 360deg);`
  );

  // Bars fill heaviest weight first, which is the order they matter in.
  const fillRank = $derived(
    Object.fromEntries(
      [...(data?.dimensions ?? [])]
...
{#snippet meter(percent: number, colour: string, delay: number)}
  <div class="h-1.5 w-full overflow-hidden rounded-full bg-[#17213a]">
    <div
      class="lu-fill h-full rounded-full"
      style="width:{percent}%; background:{colour}; transition-delay:{delay}ms"
      use:arrive
    ></div>
  </div>
{/snippet}
...
<section class="grid gap-5 lg:grid-cols-[19rem_minmax(0,1fr)]">
    ...
    {:else if data}
      <div class="lu-enter">
        <div
          ...
          use:arrive={() =>
            ring.set(score, { duration: reducedMotion() ? 0 : SETTLE })}
        >
          <div
          ...
          >
            <CountUp
              value={score}
              class="lu-d-xw text-[42px] leading-none text-white"
            />
  ...
  >
    ...
    <!-- Height follows the content, so switching tabs or finishing loading
         resizes the card smoothly instead of jumping. -->
    <div class="mt-5" use:autoHeight>
      <div>
        {#if loading}
          <div class="space-y-4">
            ...
            <div class="lu-enter">
              {#if activeTab === 'dimensions'}
                <p class="text-[13.5px] text-[#94a3b8]">
                  Each bar is the dimension score. The weighted contribution to
                  the composite is on the right.
                </p>
                <div
                  class="mt-4 divide-y divide-[#17213a] border-t border-[#17213a]"
                >
                      ...
                      >
                        <div class="flex min-w-0 items-baseline gap-2.5">
                          <span
                            ...
                            style="color:{urgencyColour[r.urgency]}"
                            >{r.priority}</span
                          >
                          <h3
              ...
              {:else}
                <p class="text-[13.5px] text-[#94a3b8]">
                  Weights are fixed per stage, not tuned per startup, so scores
                  stay comparable.
                </p>
                <div
                  class="mt-4 divide-y divide-[#17213a] border-t border-[#17213a]"
                  ...
                  {#each data.weightRationale as w (w.key)}
                    <div class="py-4">
                      <div class="flex items-baseline justify-between gap-4">
                        ...
                        <p
                          class="lu-num text-[14.5px] font-semibold text-[#818cf8]"
                        >
                          {Math.round(w.weight * 100)}%
                        </p>
                      </div>
                      <p
                        class="mt-1.5 text-[13px] leading-[1.6] text-[#94a3b8]"
                      >
                        {w.rationale}
                      </p>
                    </div>
