<script lang="ts">
  ...
  async function loadChatHistory() {
    ...
    try {
      const response = await fetch(
        `/api/chat-history/initiatives/${initiative.id}`
      );
  ...
  async function handleSendMessage() {
    ...
    try {
      const response = await fetch(`/api/initiatives/${initiative.id}/refine`, {
        method: 'POST',
        headers: {
  ...
  <Dialog.Content class="h-[90vh] max-w-[1200px]">
    <div class="flex h-[80vh] gap-0">
      ...
      <div class="flex w-1/2 flex-col border-r border-border p-6">
        <h1 class="lu-h2 mb-4">Refine Initiative</h1>
        <div
      ...
      <div class="flex w-1/2 flex-col p-6">
        <div class="flex justify-between">
          <h2 class="lu-h2 mb-4">Initiative Details</h2>
          <Button
            size="sm"
            variant="destructive"
