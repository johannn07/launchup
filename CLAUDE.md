...
- **Scored dimensions:** TRL, MRL, RRL, ARL, ORL — Technology, Market, Regulatory, Acceptance (the SRS/SDD call it *Adoption*), Organizational — plus Investment (IRL), which the specification does not define and which sorts last. **That sequence is the canonical order** (SRS §1.2; SDD §2.2 and the `urat_questions` schema — the §1.3 acronym lists are alphabetical and say nothing about order) and is declared in exactly two places: `ReadinessType` in the backend, `readiness-dimensions.ts` in the frontend. Anything showing dimensions in sequence derives from one of them.
