...
| 4. Leniency bias correction (adversarial prompting, normalization) | 🟡 Normalization + audit trail built. **Adversarial prompting is now real on the application-summary path** (SO 4.2, measured 2026-08-18) — a field-ordered `responseSchema` forces unmet criteria before the summary. The readiness-**scoring** path is unchanged and still post-hoc review. **SO 4.4 is complete end to end (2026-08-23)** — rule recalibrated to `ratio < 0.75`, validated on a held-out run, the verdict renders in all four Manager dialogs, and approving a flagged application now requires an acknowledgement that is enforced server-side and recorded in `activity_logs` |
...
- **SO 4.2's mechanism is measured; SO 4.4 is now complete.** `generateStartupAnalysisSummary` runs a field-ordered `responseSchema` (`unmet_criteria` → `critical_risks` → `summary`) behind `AI_ADVERSARIAL_SUMMARY_ENABLED`. Against the shipped prompt (10/12 calls, 2 adversarial cells lost to 503 model overload): 100% schema adherence, 3 vs 1 mean critical observations, 4 mean unmet criteria where the baseline arm has no criteria field at all. The original `flagged = criticalCount === 0` fired **0 times in 10 summaries in both arms** — the legacy prompt mandates a risk sentence, so the rule could not fire against the prompt it polices. **Replaced 2026-08-18 with `flagged = ratio < 0.75`** (note the direction: a *low* critical share is the lenient case) and validated the same day on held-out generations — baseline 5/5 flagged, adversarial 0/4, perfect separation — with a third confirmation from the 2026-08-20 run (0/10 flagged, ratio 1.00 throughout). **The verdict reaches the Manager** as a badge in all four dialogs, and **since 2026-08-23 it has an action**: approving a flagged application requires an acknowledgement, enforced in `approveApplicant` and recorded with the Manager's identity. **The differentiation guard was rebuilt, not merely re-run** — the count-based verdict was retired as degenerate and replaced by field-overlap scoring against a rule pre-registered a day before the data; the 2026-08-20 run returned a quotable `FAIL - uniform` whose cause is **noise-floor instability on one document** (AgroLink 0.800, MediSync 0.424), not uniform harshness. `TODO_CHECKLIST.md` §0 and `backend/measurement/README.md`.
