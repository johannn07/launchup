...
- **SO 4.2's mechanism is measured; SO 4.4's flag rule is measured wrong.** `generateStartupAnalysisSummary` runs a field-ordered `responseSchema` (`unmet_criteria` → `critical_risks` → `summary`) behind `AI_ADVERSARIAL_SUMMARY_ENABLED`. Against the shipped prompt (10/12 calls, 2 adversarial cells lost to 503 model overload): 100% schema adherence, 3 vs 1 mean critical observations, 4 mean unmet criteria where the baseline arm has no criteria field at all. But `flagged = criticalCount === 0` fired **0 times in 10 summaries in both arms** — the legacy prompt mandates a risk sentence, so the rule cannot fire against the prompt it polices. `ratio >= 0.75` is the calibrated replacement (the arms do not overlap) and is not implemented. **The differentiation guard did not pass**: both arms `FAIL - uniform`, with the adversarial arm saturated at the three-sentence ceiling, so the run cannot separate genuine overcorrection from instrument ceiling. `TODO_CHECKLIST.md` §0 and `backend/measurement/README.md`.
