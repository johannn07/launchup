...
- [x] ✅ **OBJECTIVE · M · Verified-knowledge corpus built (tasks 1-8, 2026-07-28) — `verifiedFrameworks`/`businessModels` are no longer hardcoded `[]`**
  ...
  **What the corpus is, precisely — 64 rows total, every row carries a `provenance` field, none of it is silently mixed:**
  ...
  - **10 `business_framework` rows** (Business Model Canvas, Lean Canvas, market sizing, unit economics, customer discovery, go-to-market, PH regulatory basics, IP basics, evidence standards, org design) — a mix of `framework-derived` (attributed to Osterwalder & Pigneur, Maurya, Blank, a16z) and `authored` (no external citation exists for that content).
...
- [ ] 🔴 **OBJECTIVE · S · Grounding measurement (Task 9, `measurement/measure-grounding.js`, 2026-07-28) — the SDD's rubric-retrieval mechanism is settled; the headline hallucination/differentiation numbers are not measured at all**
  ...
  **The three generation arms (baseline vs `ragCorpus` semantic vs deterministic) did not run.** Every attempt on 2026-07-28 hit `GenerateRequestsPerDayPerProjectPerModel-FreeTier = 20` for `gemini-3.6-flash` — a **20-requests-per-day hard cap**, confirmed from the 429 body, not a pacing issue. So: **the unsupported-claim rate, the rubric-term grounding rate, and the differentiation gap under the corpus are all still unknown** — not "the corpus doesn't help," but "nobody has been able to ask the question yet with this model on the free tier." Objective 1's headline claim (grounding reduces hallucination) remains unverified either direction.
  **Work:** re-run `node measurement/measure-grounding.js` on a day with fresh `gemini-3.6-flash` quota (ideally the very first calls of the day), or spread the three arms across separate days. Full results and caveats in `measurement/README.md`.
  **Task 10 addendum (live verification, 2026-07-28):** confirmed against the *running server*, not just the standalone harness, that the shipped `deterministic` mode's rubric text reaches a real assembled prompt (RNS generation, AgroLink PH, TRL 2 + TRL 3 verbatim) and that `AI_RAG_CORPUS_ENABLED=false` removes it. This closes the "does the deviation even change the prompt" question but not the "does it change the model's output" question — the same `gemini-3.6-flash` daily quota blocked a live generation success on the corpus-on arm the same way it blocked Task 9 (one corpus-on RNS call did assemble the correct prompt and then hit the 429; a corpus-off call on the same day happened to succeed). **Metrics 1-3 (unsupported-claim rate, rubric-term grounding rate, differentiation gap) remain unmeasured** — same open item as above, not resolved by Task 10.
