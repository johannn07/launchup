...
| SO 4.4 approval gate, 4c flag gating, mentor baseline revision | `fix/silent-controls` (local) |
...
- [ ] 🧹 **DEBT · S · `riskNumber_normalized` has no consumer anywhere** — *found 2026-08-23*
  `generateRoadblocksFromPrompt` appends `riskNumber_normalized` / `riskNumber_z` to every roadblock and **nothing in either app reads them** (`target_level_normalized` has exactly one consumer, `rns.service.ts:433`; these have none). Computed, appended, discarded — the same shape as the three defects fixed on `fix/silent-controls`.
