...
| **1b** RAG pipeline grounding calls in retrieved context | 🟡 **Corpus built and live-verified; measured at n=3, and the measured effect on level placement is negative** — the corpus arm's placement error is roughly double baseline's (MAE 1.36 vs 0.78; within-one-rung 13/36 vs 30/36), and the cause is now localised to per-dimension rubric calibration rather than prompt volume, which was tested directly and refuted. Fabrication remains saturated at 0/15 on every arm, so hallucination itself is still unmeasurable with this probe. See the measurement item below and `measurement/README.md` | Real semantic retrieval as of 2026-07-27: `gemini-embedding-2` at 768 dims, vectors written to `vector_embeddings`, ranked by pgvector `<=>`, floor calibrated to 0.78. `AI_RAG_STRATEGY=keyword\|semantic` makes the old token-overlap path an explicit baseline arm rather than something mislabelled as RAG. **The corpus (tasks 1-8, 2026-07-28)** is 54 readiness-rubric rows + 10 business-framework rows with per-row provenance — see below for exactly what is and is not externally sourced. `AI_RAG_CORPUS_ENABLED`/`AI_RAG_RUBRIC_MODE` gate it. **What's still open:** the SDD §3.2 rubric-retrieval mechanism doesn't retrieve anything against this corpus (Task 9, settled), and whether the shipped deterministic deviation actually reduces hallucination is **still open** — the 2026-07-29 n=1 measurement succeeded but measured a ±1.0 noise floor larger than any effect it could resolve, found the hallucination probe saturated at 0/15 across all arms, and showed `sdd-semantic` sends a prompt byte-identical to `baseline`. **Both probes were redesigned; two clean reps now exist** (2026-07-30 + 2026-08-03, **n=2 pooled**). The corpus arm scores worse on both remaining metrics in *both* reps — and, more informatively, it reproduces nearly the same wrong per-dimension placement each time (`+2` on T/M/A, `−2` on O/R/I), which retracts the earlier "54 rows destabilise placement" hypothesis in favour of a **systematic displacement traceable to the rubric text's own calibration**. Still measured only through the levels probe (all 54 rows), not through production's 12-row retrieval path, so it does not yet establish production behaviour. Needs a third rep for metric 3. Task 10 (2026-07-28) confirmed live, against the running server and real Neon, that the corpus reaches the assembled prompt and that disabling it removes the rubric section — see below |
...
- [ ] 🟡 **OBJECTIVE · S · Grounding measurement (`measurement/measure-grounding.js`) — measured at n=3 across five arms, and the answer is negative: the rubric corpus makes level placement *worse*. What is left is corpus calibration, not more measurement**
  ...
  **UPDATE 2026-08-03 — rep 2 ran; n=2 pooled, and the corpus arm's error turned out to be reproducible.**
  ...
  Both reps agree in direction on both scored metrics: the corpus arm places levels worse and differentiates worse. **The new information is not the means — it is that the corpus arm returns nearly the same wrong placement twice.** MediSync's per-dimension signed deltas are `+2 +2 +2 −3 −2 −2` in rep 1 and `+2 +2 +2 −2 −2 −2` in rep 2, while `baseline` moves around more between reps than the corpus arm does.

  **This retracts the previous "volume destabilises placement" hypothesis** — reproducibility is the opposite of destabilisation. What the corpus arm does is **displace placement systematically**: +2 on Technology/Market/Acceptance, −2 on Organizational/Regulatory/Investment. That points at the **rubric text's own calibration** (the O/R/I rungs appear to demand more evidence than the model's unaided prior, the T/M/A rungs less) rather than at prompt volume — a live hypothesis, not a demonstrated cause, but one that is per-dimension and therefore correctable in the corpus rows. `within one rung` is the sharpest number: baseline 21/24 vs corpus 8/24, and since the corpus arm's exact count is *also* 8, **every non-exact corpus placement is off by more than one rung.**

  **Metrics 1 and 3 remain one finding read two ways** — both derive from the same `levelCalls` array, and the displacement pattern mechanically raises MAE *and* compresses the early-vs-mid gap. Not two corroborating lines of evidence.

  **Noise floor at n=2:** 0.33 MAE / 0.17 gap points between `baseline` and `sdd-semantic`, whose byte-identity was re-verified this rep by diffing the `--dry-run` prompts (identical, same md5). Still a single paired difference, so no "N× the noise" multiplier is licensed.

  **Separate finding, not a corpus effect:** *every* arm overshoots Acceptance (+1.0 to +2.5 pooled, both startups), including the two that see no rubric text. Points at the seeded Acceptance ground truth or the seeded documents, not the corpus. Worth checking against `seed-demo-full.js`.
  ...
  **UPDATE 2026-08-04 — n=3 is complete and balanced, the volume hypothesis is refuted *empirically*, and the displacement hypothesis is confirmed and sharpened.** 20/20 of the day's quota, three files: `2026-08-04-rep3-refill.json` (2 calls), `2026-08-04-titles-arm.json` (12), `2026-08-04-bare-arm.json` (6).

  **The missing cell is refilled, so the n=3 pooled figures are now like-for-like** (deviation 18/18 against baseline's 18/18). The caveat above — "do not quote the n=3 pooled MAE (1.23)" — is **discharged**: the balanced number is **1.36**.
  ...
  **Volume is refuted by experiment, not by inference.** The retraction recorded above rested on "reproducibility is the opposite of destabilisation" — a reasonable argument but not a test, since a volume effect would also reproduce. Two new arms test it directly as a ladder: `deviation-titles` drops each row's body, `deviation-bare` drops the provenance suffix too, both holding all 54 keys so level coverage is unchanged and the true level is still never leaked. **An 87% cut in block size leaves aggregate placement flat and bad.** (Trimming *levels* instead would have been the wrong experiment — it removes the correct answer for any startup at level 2–4, degrading placement for an unrelated reason.)

  **The per-dimension picture is where the finding actually lives.** Mean signed error, + = placed too high:
  ...
  - **Organizational and Investment are volume-invariant** — every corpus arm at −1.17 and −1.00, identical to two decimals across the whole cut, while baseline places both *too high* at +0.67. The corpus flips the sign and the flip does not care how much text is sent. **This is the displacement hypothesis confirmed, and it now has a much stronger basis than the n=2 reproducibility argument: the effect survives an 87% change in everything except the rubric's meaning.** It is per-dimension and therefore correctable in the corpus rows — that is where the next work belongs.
  - **Technology and Acceptance move the opposite way and *do* track volume**: +1.00 → +2.50 and +2.17 → +3.00 as bodies are stripped. Removing text made over-placement **worse**, which reads as a bare title being an aspirational label with no criteria attached. So do not "fix" this by shortening the rubric — the body was the thing restraining it.
  - The all-arms Acceptance overshoot flagged above persists (baseline +2.00), so that remains a ground-truth or document question, not a corpus one.

  **`--only-probe=` added.** Metric 2 has been saturated at 0% on every arm since the 2026-07-30 redesign, so half of every rep bought nothing; narrowing to the levels probe halved the cost of the only discriminating metric and is the sole reason a third ladder point fit inside the cap. Also: adding a fourth arm made `--only-arm=deviation` ambiguous, which would have silently run two arms and doubled the spend — ambiguous prefixes now hard-error and name the candidates. Harness tests 91 → 103.

  **Still the limit to quote:** every number here is the *levels* probe, a harness construct. Production does not ask the model to assign readiness levels — mentors set them. This is a direct negative result for 1b's **assessment** claim and says nothing about RNA generation quality, where metric 2 has never produced a signal on any arm.

  🛑 **UNDER REVIEW 2026-08-05 — do not quote the negative conclusion above, and do not do the recalibration work below yet.** The reference every number in this item is scored against — the seeded `StartupReadinessLevel` rows — is contradicted by the startup documents the model is shown, in at least five cells. Examples, all checkable from the document text alone: MediSync's seeded Market 4 requires *"no prospect has yet indicated a specific willingness to pay"* against a document stating PHP 5,000 monthly recurring revenue; its seeded Organizational 4 requires a *"first full-time hire beyond the founders"* against *"team grew to 3 founders"*; its seeded Investment 3 requires *"a written funding plan document with a stated amount"* against a document mentioning no funding activity at all; AgroLink's seeded Acceptance 1 requires that *"no user has interacted with the product in any form"* against a paper prototype tested with 3 cooperatives.
  Re-scoring the same 30 collected levels calls against a reference derived from the documents **reverses the direction on every arm** (`measurement/audit-ground-truth.js`, spends no quota, and reproduces the seeded figures above exactly — so the scoring path is validated and only the reference differs). Corpus arm 1.36 → **0.28** MAE and 13/36 → **36/36** within one rung; `baseline` 0.78 → 0.97. The "systematic displacement" on O/R/I inverts too: baseline places those dimensions **+1.67 / +0.83 / +1.67** rungs too high — inventing investor conversations and org charts the documents do not mention — and the corpus arm sits at −0.17 / 0.00 / 0.00.
  **The reference-free result is the one to quote, and it is the strongest number this study has produced.** Every reference here is contestable, and a *model*-set one is worse than contestable — an adjudicator reading the document with the full rubric ladder in front of it is approximately the `deviation-deterministic` condition, so its agreement with that arm is close to circular. So the claim was restated in a form that needs no reference at all. Three rungs require an artifact class **neither document mentions anywhere**: ORL 3+ needs a non-founder contributor, RRL 3+ needs counsel engaged, IRL 3+ needs a written funding plan. The absences are **asserted at run time, not trusted** (`verifyAbsences`, and the ceilings are set one rung more generous than the documents support, so this is a lower bound):
  ...
  Baseline places MediSync's Investment at 4–5 — *"initial investor conversations"*, *"angel funding secured"* — for a document containing no funding token of any kind. This is an unsupported-claim rate measured directly against the source document, which is Objective 1b's actual claim and doubles as an Objective 4 leniency result. It is **directional**: it catches over-placement into absent evidence and is silent on under-placement.

  **The reference is corrected as of 2026-08-05, in the app and in the harness.** `src/demo-readiness-levels.ts` is now the single source both seeders read (they previously held separate copies, which is how they drifted), and `measure-grounding.js` mirrors it — a test parses the TS source and fails if either side moves alone. AgroLink is now `T2 M3 A3 O2 R1 I1`, MediSync `T6 M5 A5 O2 R1 I1`. Applied to Neon via `node seed-demo-full.js`; 8 rows changed, 0 skipped, and re-running reports 0 changes. `--check-levels` reports what would change and exits before writing anything. Rows whose remark is no longer the seeder's own are left alone, so a mentor's rating is never overwritten by a seed value.
  **Consequences to know:** every fingerprint changed (levels are inside `common`), so runs collected before the correction correctly refuse to pool with runs after it — the RNA prompt itself changed. `audit-ground-truth.js`'s `SEEDED` is deliberately **frozen** at the old values, since that is what the collected runs were scored against, and a test asserts it does *not* track the harness. Composites moved: AgroLink **17 → 26** (crosses the 25 tier threshold), MediSync **40 → 41** under healthtech. `readiness_evaluations` rows still hold pre-correction composites — the same un-backfilled-rows item §3 already carries.

  **Still not established, which is why this is a hold and not a rewrite:** that the corpus places *closer to the true level*. That needs a reference, and the derived one was written after the results were known. A blind adjudication worksheet is at `measurement/data/ground-truth-adjudication.md`. The volume finding is unaffected in direction — stripping the rubric bodies still sends MediSync to TRL 9 on every rep, so the bodies remain load-bearing.
  **One caveat that cuts the other way, and it is new:** on the *RNA* probe, where the level is supplied rather than inferred, the corpus arm wrote *"The venture has drafted a funding plan (IRL 3)"* — asserting the rubric's evidence requirement as a fact about the startup. A wrong supplied level turns rubric text into fabricated evidence, and the 0/15 fabrication probe does not catch this class.

  **Work — calibrate the O/R/I rubric rows, then re-measure.** *(Superseded pending the review above — the premise that the seeded reference is correct is what is in question.)* The next experiment is not more reps or another arm; it is editing the Organizational, Regulatory and Investment level descriptors so their evidence bar matches the seeded ground truth, then re-running `deviation-deterministic` alone:
  ```bash
  node measurement/measure-grounding.js --only-arm=deviation-deterministic --only-probe=levels --reps=3 --out=measurement/results/<date>-recalibrated.json
  node measurement/measure-grounding.js --merge measurement/results/*.json
  ```
  That is **6 calls**, well inside one window. Note the corpus edit changes the corpus content hash, so the recalibrated arm will correctly refuse to pool with the runs above — that is the fingerprint guard working, not a failure.
  ...
  The window resets **15:00 Philippine time** (midnight US Pacific), and a run started before 15:00 draws on the *previous* window — rep 2 ran at 01:15 on 08-03 and spent from the 08-02 window. `--merge measurement/results/*.json` also works and gives identical numbers: **verified 2026-08-03 on Windows/Git Bash** that `fs.globSync` expands it and that the fingerprint guard refuses the superseded `2026-07-29-rep1.json` with an explicit per-group "Not pooled" list rather than silently averaging it in. Both of the probe redesigns this section used to flag as "indicated" are **done**, and as of 2026-08-04 so are the reps: n=3 is complete and balanced across five arms. **What remains is editing the O/R/I rubric rows, not collecting more data.** Full detail and caveats in `measurement/README.md`.
