...
| **Eleven non-reactive state variables + the bias-audit `populate` they exposed** — OCR preview, tier editing, bias-audit overrides, elevate picker, landing header | `fix/non-reactive-state` — merged `2870e9e`; 380/380 backend, `svelte-check` 113/11 |
...
| **Kanban drops anywhere in a column; refine chats retry a busy Gemini and stop rewriting on non-requests** | Three branches merged to local `master` `d08863d` 2026-09-26, awaiting John's test and push — see `SESSION_NOTES.md` 2026-09-26 |
...
| **Broken functionality (§2)** | In progress — **19 of 26 fixed**. 2026-09-26: Kanban drop zone and the two refine-chat defects (Gemini 503 surfaced as 500; non-requests rewrote the item) fixed on local `master` `d08863d`, one live check owed. 2026-09-18: the eleven non-reactive state variables fixed, plus the missing `populate` they exposed on the bias-audit override (`2870e9e`); the landing header's scroll listener added as a new open item. 2026-09-17: three UI defects fixed and deployed (avatar-menu lock, applications dialog state, no overview for unqualified startups); **the same non-reactive-state defect found in eleven more variables — open, possibly visible on admin pages**. Earlier: 15 of 21 fixed — 14 merged to `master` 2026-09-07 (`level_criteria`, `readiness_levels.name`, remove-member, the unguarded controller), all four tested by John first, three of them carrying a wrong diagnosis — impact, scope and risk; see `SESSION_NOTES.md` 2026-09-07. The dimension-order defect added and fixed 2026-09-07 on `fix/readiness-dimension-order`, unblocked by reading the SDD. **7 open.** The `supportRatio` defect stays open as a measurement problem, its false "Verified" badge removed without touching the threshold |
| **Incomplete features (§3)** | Decided 2026-08-07 — cut, don't defer; 9 items still open, the deletions not yet executed. Added 2026-09-26: stable item numbers on the Kanban boards, on hold for John's decision |
...
| **2a** Multi-tier classification schema | 🟢 Built | `TierConfig` entity + `/admin/tiers` UI + threshold logic (`readiness.service.ts`). **Seeded 2026-09-04** — `tier_configs` was empty on every DB (nothing ever seeded it), so the page showed "No tiers configured" while scoring silently used a hardcoded fallback ladder. Both now read `SEED_TIER_CONFIGS` (`readiness/readiness.tiers.ts`); the seeder is create-only so Manager edits survive a reboot |
...
| **3a** OCR of handwritten text | 🟢 **Implementation complete. Accuracy measurement explicitly scoped out to 3c on 2026-09-05** — a decision, not an omission | **Built and live-verified end to end (2026-08-22):** a real phone photo of handwriting returns a near-verbatim 1017-character transcription, and field confidence tracked reality — 6 `verified` for fields on the page, 2 `low` for fields the model invented. Three defects fixed the same day: production stored **Tesseract's** transcription rather than Gemini's (the guard was the inverse of its own comment); `fieldConfidence` was `text.length < 40`, which graded the model's compliance with a prompt that *mandates* 40 characters; and the replacement rule was **circular on the fallback path** — a field extracted from Tesseract's garbage scored 1.00 against that same garbage and rendered green. **The distinction that matters in front of a panel: there is no missing code here** — unlike 3b. **Scope decision, 2026-09-05:** CER moves to 3c and is *not* claimed here. **Say "we scoped the accuracy measurement out", never "3a is complete"** — a panel hears the difference, and "how accurate is your OCR?" is the most predictable question this objective attracts. The honest answer is that the harness exists, the run is stored, and the transcription pass was not spent. **What *was* measured 2026-09-05** (`measure/ocr-accuracy`, 10 pages, 2 writers, 12 calls): the field-confidence threshold — a layer downstream of the OCR, not the OCR itself. Result is **negative for the shipped value** and tracked as an open defect below — at 0.5 the layer badges **18 of 26 invented fields green "Verified"** (specificity **30.8%**). **No replacement ships:** the pooled best (0.7879, J 0.833) passes the pre-registered gate but the confound-free arm fails it (J 0.438, spec 43.8%), and the per-field means say why — an *invented* `scope` (0.632) outscores a *grounded* `methodology` (0.575), so a single global threshold compares quantities that are not comparable. See `measurement/README.md` |
...
| **3c** Accuracy evaluation (Character Error Rate + SUS) | ⚪ Research task — **now explicitly owns 3a's CER, scoped out of 3a on 2026-09-05** | Not a code deliverable — needs a ground-truth dataset. **CER is no longer blocked on tooling: the harness is built, gated and tested** (`measure-ocr-accuracy.js --score`), the 10-page/2-writer run is stored, and the spans are drawn by seed. It needs **30–50 minutes of human transcription** into `measurement/data/ocr-reference-spans.md` and **zero quota** — nothing else. SUS still needs the user study. **Say "we scoped the accuracy measurement out", not "3a is complete"** |
...
- [x] ✅ **OBJECTIVE · S · Metric 6 (redundant-need rate) — RETIRED 2026-09-05 on its own pre-registered stopping rule, after a clean 12/12 run.**
  ...
  **The 12-call run happened 2026-08-23** (`measurement/results/2026-08-23-rna-redundancy.json`, 12/12 calls, no 429s/503s/retries). **Prediction 1 failed: `redundantRate` is 0 on every arm under both `truth` and `deflated`** — the positive control never fired, which by the pre-registered rule voids the run as a model result. Prediction 2 is untestable as a consequence (no arm difference exists to read). **The pre-registration's own inference from a failed control was itself wrong** — reading the deflated-condition text shows the manipulation never induced the target behaviour (every arm's recommendations stayed correctly anchored to the source document), so the detector had nothing to catch, not "the detector is blind." The honest claim is narrow: the model did not make this error in these 36 observations — not "the detector works," not "the model is robust to a deflated level." The two uncaught classes named in the pre-registration (passive/postposed acquisition, acquisition verbs outside the frozen list) remain completely untested. **`mentioned` equals `unclassified` in all six cells** (baseline truth/deflated 2/6, 1/6; sdd-semantic 2/6, 2/6; deviation-deterministic 1/6, 1/6) — every clause naming an already-evidenced artifact failed to classify as an acquisition request; none was binned `recommended` and then rejected by the gate. Consistent both with the model never making the error and with the classifier being unable to read these constructions — this run can't separate those two. Also observed, n=1, not this run's question: metric 5 `asserted` 0/6 every arm both conditions, `mentioned` on `truth` — baseline 1/6, `sdd-semantic` 2/6, `deviation-deterministic` 4/6. Full table and verdict: `measurement/README.md`, metric 6, "Result, 2026-08-23."
  ...
  > **Corrected and redesigned 2026-09-04 — zero quota spent.** Re-scoring the stored 2026-08-23 text through the same `lib/redundancy.js` reproduces all four recorded columns exactly and exposes a fifth the harness never emitted: **4 clauses were binned `recommended` and downgraded to `scoped` by the acquisition gate**, all four correct rejections of the *"needs to move **from paper prototype** to…"* shape. With 14 `recommended` clauses in the same run's metric-5 output, **the classifier demonstrably reads the model's recommendation register and the gate demonstrably acts on real verdicts** — so the README's recorded ambiguity ("model never made the error" vs "classifier is blind") resolves in favour of the first. The claim it replaces was invisible for eleven days because `scoreRedundantNeeds` computes `scoped` and nothing aggregates, prints or persists it.
  >
...
- [ ] 🔒 **SEC · M · Add ownership checks to startup detail endpoints (IDOR)** — *judgement call, see triage*
  ...
  **Fix:** a reusable guard or service helper asserting the requester owns / is a member of / mentors the startup, unless Manager. **Done for the startup module 2026-09-04** — `canAccessStartup` (`startup/startup-access.ts`) is that helper, applied through `assertCanAccessStartup` to twelve routes covering every per-startup read and the capsule-proposal write; `/all` and both rankings moved to `AdminGuard`. Live-verified across all three roles. **Still open elsewhere:** the per-startup reads in `rna`, `rns`, `initiative` and `roadblock` are unguarded, and the generalized `RolesGuard` + `@Roles(...)` decorator was not built — this is a service-level assertion, not a guard.
...
- [ ] 🔒 **SEC · S · Align cookie lifetime with token lifetime**
  Cookie `maxAge` = **5 hours** (`(auth)/login/+page.server.ts`, mirrored in `(auth-manager)/manager-login/+page.server.ts`) vs JWT `expiresIn: '24h'` (`auth.module.ts:19`). The token stays valid for 19 hours after the browser stops sending it, so a leaked token outlives the visible session. See the matching bug in §2.
...
- [x] ✅ 🔒 **SEC · S · Reconsider client-side role checking on the manager login** — *done 2026-09-04*
  The old `(auth-admin)/admin-login/+page.server.ts` base64-decoded the JWT payload and rejected non-Admins without verifying the signature. Both login actions now verify with `jose` through `frontend/src/lib/server/auth.ts`, sharing `hooks.server.ts`'s secret.

---

## 2. Broken functionality

- [x] ✅ **BUG · Kanban cards only dropped on part of a column** — *found and fixed 2026-09-26 (`fix/kanban-drop-target`, merged to local `master` `d08863d`)*
  The dnd zone was only as tall as its cards, so a short column rejected drops over most of its visible area — including a card returning to the column it just left. The zone now fills the column body (`kanban-new.svelte`, `column.svelte`). Also: renumbering is awaited before the refetch, and Roadblocks/Initiatives renumber once per drop instead of twice. Live-verified on RNS and Roadblocks; John confirmed. Initiatives' identical change not dragged — no items on MediSync.

- [x] ✅ **BUG · Refine chats: a busy Gemini showed "API Error: 500", and any message rewrote the item** — *found and fixed 2026-09-26 (`fix/refine-chat-ai-busy`, `fix/refine-chat-non-instructions`, merged to local `master` `d08863d`)*
  Not mentor-specific: every failed refine run in `ai_generation_runs` was Gemini **503 "high demand"**. The four refine calls now retry a 503 twice and return a readable 503; 429 is never retried; the run log records the upstream error as `cause`. Separately, all four prompts treated a message naming no field as "refine everything" — a shared rule (`ai/refine-chat.ts`) now makes a greeting, test or question change nothing, and "Here's my suggestion…" renders only for a real rewrite.
  ⚠️ **Owed:** a live check that a real request ("Make it shorter") still rewrites — three attempts hit Gemini 503s. "bro" verified live.

- [x] ✅ **BUG · Eleven more state variables were plain `let` in runes components** — *found 2026-09-17, fixed 2026-09-18 (`fix/non-reactive-state`, merged `2870e9e`)*
  Same defect as the Applications dialog: reassigning them never re-rendered. All eleven now use `$state(...)` — `admin/ocr-documents`, `admin/tiers`, `admin/ai/bias-audits`, `overview/elevate`, `landing/Header`. `svelte-check` warnings **22 → 11**, none of this class left; errors unchanged at 113.
  **Fixing them exposed a second defect, fixed in the same branch:** `overrideBiasAudit` returned the audit without `populate: ['startup']`, so the row rendered "Startup #Unknown" once the list actually re-rendered. Stored data was never wrong. Test-first, 380/380 backend, proven read-only on the real ORM.
  ⚠️ **Claude verified only the landing header** (scroll event with `window.scrollY` stubbed, fixed vs stashed original). Tiers, AI Bias and Elevate are John's to click through.

- [ ] 🐞 **BUG · S · The landing header's blur never fires from real scrolling** — *found 2026-09-18, unfixed*
  `landing/Header.svelte` listens for `scroll` on `window` and reads `window.scrollY`, but `/` scrolls inside a container, so the window never moves (`document.documentElement.scrollHeight` is 1). The `$state` fix makes the header update when a scroll event does arrive; nothing delivers one. Attach the listener to the scrolling container, as `shared/header.svelte` already does via its `scrollContainer` prop.
...
- [x] ✅ **BUG · Readiness dimension order — settled against the specification and unified** — *found 2026-09-07 by John, fixed 2026-09-07 (`fix/readiness-dimension-order`)*
  ...
  **It was worse than recorded: eight declaration sites, four different orders.**
  ...
  | Site | Was | Visible |
  |---|---|---|
  | `Application.svelte:71` steps + `:280` markup | T M R A O I ✅ | apply wizard |
  | `Pie.svelte:5`, `BarChart.svelte:7` | T M R A O I ✅ | charts |
  | `readiness-type.enum.ts` → `DIMENSION_ORDER` → `GET /readinesslevel/rubrics` | T M **A O R** I | rubric guide |
  | `utils.ts getReadinessTypes()` → assessment page + 2 RNS dropdowns | T M **A O R** I | yes |
  | `assessment-questions.ts` URAT seed bank | T M **A O R** I | no — filtered by name |
  | `readiness-baseline.ts READINESS_TYPES` → readiness-level tabs | T **A M** O R I | yes |
  | `utils.ts ReadinessType` enum | T M A **R O** I | no — type-only |

  **Fixed by making one source canonical per app**, not by patching seven lists: the backend enum (`DIMENSION_ORDER` and the rubric endpoint already derived from it), and a new `frontend/src/lib/readiness-dimensions.ts` that the other three frontend sites now import. `getReadinessTypes()` keeps its original ids **pinned by name rather than derived from position**, so a future reorder cannot silently remap them — nothing reads `id`, both consumers select on `name`.
  **Presentation only.** No stored value changes; `startup.service.ts:1429` and the corpus specs are keyed lookups or `.sort()`ed, so neither is order-sensitive. The URAT seeder is guarded on emptiness, so the reordered bank changes nothing on an existing database — only a fresh one.
  **Verified:** backend **375/375**, `svelte-check` **113 errors / 15 warnings**, both identical to `master` measured on the same machine (the "14 warnings" in older notes is stale). Live against Neon, `GET /readinesslevel/rubrics` returns all 54 rows as Technology → Market → Regulatory → Acceptance → Organizational → Investment; the four frontend sources were read out of the running Vite module graph and all agree.
  ⚠️ **Not fixed here, and each is its own decision:** the SDD calls ARL **"Adoption** Readiness Level" while the code says "Acceptance" everywhere — including as a stored `readinessType` value and in the RAG corpus keys, so that rename carries a data migration. And **IRL is not in the specification at all** (see §0); keeping Investment last is a superset of the spec order, which is why it does not block this.
...
- [ ] 🐞 **BUG · M · `supportRatio` cannot separate grounded from invented fields** — *measured 2026-09-05; the false claim removed 2026-09-07 (`fix/field-confidence-claim`), the metric still open*
  ...
  **What remains open is the metric itself**, and the warning below still stands in full — do not "fix" it by raising the number.
  ...
  the model to invent absent fields, so this is the layer that exists to catch
  that behaviour failing to catch it, in production, today.
  ⚠️ **Do not "fix" this by raising the number.** The pooled sweep's best
  (0.7879) passes the pre-registered gate but the confound-free arm fails it,
  and the per-field means show why: an *invented* `scope` scores 0.632 while a
  *grounded* `methodology` scores 0.575. `supportRatio` has a field-dependent
  baseline — `title` reuses page words verbatim, `methodology` is written in the
  model's own register whether grounded or not — so **a single global threshold
  compares quantities that are not comparable.** Raising it trades one wrong
  number for another and would cost every grounded `methodology` a green badge.
  **Per-field thresholds are the obvious hypothesis and are untested** — only
  `scope` and `methodology` carry both classes here, and fitting on this run
  then reporting the fit is the forbidden move. Needs its own pre-registered
  design on new data. Full result and per-field table in `measurement/README.md`.
  **Decide before submission**, and note it is visible in the demo: a reviewer
  who uploads a sparse page sees green badges on invented content.
...
- [ ] ❓ **SCOPE · M · Kanban item numbers change when a card moves** — *found 2026-09-26, on hold for John's decision*
  The `#N` on RNS cards (and "Step #N" on initiatives) is `priorityNumber`, renumbered across the whole board in column order on every drop, so a status move changes a card's number — #6 became #7 in John's test. Roadblocks' `riskNumber` works the same way.
  **Decision:** *Leave* (the number means board position), or *Fix* on `feat/stable-item-numbers` — a separate `rnsNumber` label assigned max+1 at creation and backfilled from today's `priorityNumber`, with `priorityNumber` kept for ordering. Costs: deletions leave gaps, the number stops meaning priority, and it is a schema change. Initiatives number per RNS and need their own look.
...
- [x] ✅ 🧹 **DEBT · S · Three components carried a hardcoded, expired JWT from the previous team's app** — *resolved 2026-08-22 by deleting the three files*
  ...
  **Why it mattered:** it looked like working auth and was not, which is how the "the frontend already sends Bearer tokens" assumption survived. All three were re-verified unimported (no `index.ts` exports them) and the token re-verified expired before deletion, so no credential rotation was needed. The coupling note is resolved: the "rate applicant" scope call was settled by the 2026-08-07 §3 decision (*cut cleanly*), and the SO 4.4 work had already recorded `PendingTab` as deliberately unwired. ⚠️ The literals remain in git history.
...
- [ ] ❓ **SCOPE · M · OCR extraction cache and proposal-image storage** — *deferred 2026-09-06*
  ...
  2. **Image storage.** `OcrDocument.sourcePath` is still never written, so the handwritten proposal itself is stored nowhere. ⚠️ **Confirmed user-visible 2026-09-18:** Admin → OCR documents shows **no preview button on any row** — the control is gated on `o.sourcePath`, so the preview dialog is unreachable until this is done. `UploadService.uploadSingle` already names "OCR intake" as an intended caller and Supabase is configured — the missing piece is that `StartupModule` does not import `UploadModule`. Watch the 10&nbsp;MB cap and MIME allowlist in `validateUpload`: `checkLegibility` demands ≥1200×900, so these are deliberately large photos.
