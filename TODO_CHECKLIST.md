...
- [ ] 🐞 **BUG · S · Any `CheckboxItem` dropdown leaves the page unclickable** — *found 2026-08-25*
  bits-ui's `ScrollLock` restores `body.style.pointerEvents` **only in its effect teardown** — on unmount, not on close — and `dropdown-menu-content.svelte`'s `data-[state=closed]:animate-out` classes keep the node mounted. `body { pointer-events: none }` therefore sticks after the menu closes and the page stops responding to clicks until reload.
  **Reproduced on untouched master**: the RNS page's *View* dropdown (`show-hide-columns.svelte`). The avatar menu, which uses plain `Item`s, is unaffected.
  **Fix:** default `preventScroll={false}` in `dropdown-menu-content.svelte` — one line, fixes every dropdown at once. The RNA dimension picker already passes it per-menu (`24e1105`); the shared wrapper is deliberately untouched, being outside that change's approved scope.
