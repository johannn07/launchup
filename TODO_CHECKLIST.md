...
- [ ] 🟡 **OBJECTIVE · S · Grounding measurement (`measurement/measure-grounding.js`) — measured at n=3 across five arms, and the answer is negative: the rubric corpus makes level placement *worse*. What is left is corpus calibration, not more measurement**
  ...
  **Work — calibrate the O/R/I rubric rows, then re-measure.** The next experiment is not more reps or another arm; it is editing the Organizational, Regulatory and Investment level descriptors so their evidence bar matches the seeded ground truth, then re-running `deviation-deterministic` alone:
  ```bash
...
- [ ] ❓ **SCOPE · S · Readiness scores are clamped to 0–5 but levels run 1–9**
  ...
  **Why it matters:** a startup at level 9 scores identically to one at level 5. That directly undermines "Enhance Startup Readiness Differentiation," which is objective 2.3 of the team's SRS (see the capstone folder — `docs/SRS.md` was a repo summary of this and has been deleted, see §0 above).
