...
| **3a OCR accuracy harness — 10-page/2-writer corpus, 80 labelled observations, threshold sweep with a confound-free arm, CER primitives, zero-quota gates, and the 12-call run** | `measure/ocr-accuracy` — local, unpushed. Stage 1 done and negative for the shipped threshold; stage 2 (CER) awaits typed spans |
...
| Objective | Status |
|---|---|
| **Capstone objectives (§0)** | In progress — 1b, 1c, 2a, 2b, 2c, 4c and SO 4.4 built and measured; 1a, 3a, 3b and 4b partial; 4a is a research task, not code. **Metric 6 retired 2026-09-05** on its stopping rule. **3a's threshold measured 2026-09-05** — the shipped 0.5 is too permissive (spec 30.8%) and no replacement ships; 3c's CER half is built and gated, awaiting typed reference spans |
