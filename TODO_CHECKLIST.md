...
- [ ] ⚪ **OBJECTIVE · S · Metric 6 (redundant-need rate) — built, pre-registered, code-reviewed. The 12-call run is NOT yet made.**
  ...
  **The 12-call run is pre-registered** (`measurement/README.md`, metric 6 section — mechanism, both pre-registered predictions, the `deflated` positive control, the null-control reading rule) **but has NOT been run.** Free-tier budget is 20 calls/day; this run alone is 12. **Awaits explicit authorisation before spending quota.**
