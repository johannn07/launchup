/**
 * The URAT and calculator question banks.
 *
 * Extracted from AppService, whose generator methods were the only home for
 * this data and were reachable only through three controller endpoints that
 * were all commented out — so on any database created after the 2026-07-26
 * wipe both tables were empty and the URAT and calculator steps rendered blank
 * with no error. Seeded on boot now; see seed-assessment-questions.ts.
 */
...
export const URAT_QUESTIONS: { question: string; category: ReadinessType }[] =
  [
    {
      question:
